...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        """
        state = x, y, theta i.e. position and orientation
        beta = turning angle of car
        beta = d/L * tan(alpha) where d = distance forward,
            L = length of car, alpha = steering angle
        if beta < 0.001 approximate with straight
        R = turning radius
        R = d/beta
        cx = x - sin(theta) * R
        cy = y + cos(theta) * R
