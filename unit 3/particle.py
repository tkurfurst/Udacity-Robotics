...
class robot:
    ...
    def move(self, motion, add_noise = 1):
        ...
        if beta < beta_tolerance:
            myrobot.x = self.x + d * cos(theta)
            myrobot.y = self.y + d * sin(theta)
        else:
            R = d/beta
            cx = self.x - sin(theta) * R
            cy = self.y + cos(theta) * R
            myrobot.x = cx + sin(theta + beta) * R
            myrobot.y = cy - cos(theta + beta) * R

        theta = (theta + beta) % (2 * pi)

        myrobot.orientation = theta
...
def particle_filter(motions, measurements, N=500): # I know it's tempting, but don't change N!
    ...
    # variable (global) for original and final particle visualization
    global original, final
...
# robot finish for visulaization
robotend = final_robot

import matplotlib.pyplot as plt
# myrobot original point
plt.scatter(robotstart.x, robotstart.y, color='c', marker='D')
# particile filter start
plt.scatter([original[i].x for i in range(500)], [original[i].y for i in range(500)], color='r')
# particle filter final
plt.scatter([final[i].x for i in range(500)], [final[i].y for i in range(500)], color='b')
# myrobot finish point
plt.scatter(robotend.x, robotend.y, color='c', marker='H')
# landmarks
plt.scatter(zip(*landmarks)[0], zip(*landmarks)[1], color='g')
plt.show()
