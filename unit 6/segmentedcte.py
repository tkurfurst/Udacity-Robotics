...
class robot:
    ...
    def check_goal(self, goal, threshold = 1.0): # was 1.0
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000): # timeout = 1000
    ...
    #for r in filter.data: plt.plot(r.x, r.y, color="green", marker=".")
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # new
        plt.plot(myrobot.x, myrobot.y, color="blue", marker="x")
        if N % 10 == 0:
            for r in filter.data: plt.plot(r.x, r.y, color="red", marker=".")
        ...
        # ***
        # SEE CLASS NOTES/VIDEO
        #
        # implement a euclidean distance function
        # calculate norm from robot to the 'nearest' point on smooth path (spath)
        # use CTE = |robot - nearest spath point|
        #
        # REVIEW N PYCHARM
        #***

        # current spath segments
        x1 = spath[index][0]
        y1 = spath[index][1]
        ...
        # R == robot position vector from spath (x1, y1)
        Rx = estimate[0] - x1
        Ry = estimate[1] - y1

        # delta == spath segment vector between (x2, y2) and (x1, y1)
        dx = x2 - x1
        dy = y2 - y1
        ...
        # robot progress along delta normalized
        u = (Rx * dx + Ry * dy) / norm

        # normalized perpendicular distancet delta normalized spath segment
        cte = (Ry * dx - Rx * dy) / norm

        #print index, Rx, Ry, x1, y1, x2, y2, dx, dy, norm, u, cte
...
weight_data       = 0.1 #was 0.1
weight_smooth     = 0.1 #was 0.2
p_gain            = 3.0 #was 2.0
d_gain            = 4.0 #was 6.0
    ...
    while sum(dparams) > 0.0000001:
        for i in range(len(params)):
            ...
            for k in range(K):
                ret = main(grid, init, goal, 
                           steering_noise, distance_noise, measurement_noise, 
                           params[0], params[1], params[2], params[3]) #removed best_error
                if ret[0]:
                    err += ret[1] * 100 + ret[2]
                else:
                    err += 99999
...
####
#
# write code to print obstacles, spath, robot steps
#
####
...
for i, row in enumerate(grid):
    for j, column in enumerate(row):
        if grid[i][j] == 1:
            plt.plot(i,j, color='black', marker="o")
#plt.plot(init, color='green', marker="+")
#plt.plot(goal, color='green', marker="x")
plt.plot(init[0], init[1], color='green', marker="o")
plt.plot(goal[0], goal[1], color='red', marker="o")


for i in range(len(spath)):
        plt.plot(spath[i][0], spath[i][1], color="red", marker="+")
for j in range(len(rpath)):
        plt.plot(rpath[j][0], rpath[j][1], color="white", marker="o")
