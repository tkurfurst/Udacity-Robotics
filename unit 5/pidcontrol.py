...
def run(param1, param2, param3, plot = False):
    myrobot = robot()
    ...
    for i in range(N):
        steering = - param1 * cte_current - param2 * cte_diff - param3 * cte_integral
        cte_integral += cte_current
        cte_prior = cte_current
        myrobot = myrobot.move(steering, speed)
        res.append([myrobot.x, myrobot.y])
        cte_current = myrobot.y
        cte_diff = cte_current - cte_prior
...
run(0.2, 3.0, 0.004, True)

run(2.9, 10.3, 0.005, True)

run(1.6, 7.2, 0.16, True)

run(1.2, 4.5, 0.04, True)

run(0.9, -4.3, -0.1, True)
