...
def run(params, printflag = False):
    ...
    for i in range(N * 2):
        ...
        if i >= N:
            ...
            #err += (abs(crosstrack_error))
        if printflag:
            print myrobot, steer
    return err / float(N)


def twiddle(tol = 0.001, printflag = False): #Make this tolerance bigger if you are timing out!
    ...
    while(sum(dp) > tol):
        for i in range(len(p)):
            ...
            if error < best_error:
                best_error = error
                dp[i] *= 1.1
                if printflag: print i, dp[i], 'positive, increase'
            # otherwise, try -dp
            else:
                p[i] -= 2 * dp[i]
                error = run(p)
                ...
                if error < best_error:
                    best_error = error
                    dp[i] *= 1.1
                    if printflag: print i, dp[i], 'negative, increase'
                else:
                    p[i] += dp[i]
                    dp[i] *= 0.9
                    if printflag: print i, dp[i], 'zero reset, decrease'
        n += 1

        #format print

        #was print 'Twiddle #:', n, p, ' --> ', best_error
        #print p
        print 'Twiddle # {:3d}: [{:6.2f}, {:6.2f}, {:6.2f}] --> {:8.2e}'.format(n, p[0], p[1], p[2], best_error)
...
#sqrt(err)function
#Twiddle # 107: [  2.92,  10.33,   0.49] --> 3.61e-17
#[Finished in 0.9s]

#abs(err) function
#Twiddle # 107: [  2.27,   8.76,   0.22] --> 9.74e-11
#[Finished in 0.9s]
