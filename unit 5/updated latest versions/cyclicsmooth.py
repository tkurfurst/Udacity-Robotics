    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            ...
            for j in range(len(path[0])):
                ...
                #---
                # combine newpath update into one line!!!
                #---
                ...
                newpath[i][j] = newpath[i][j] + weight_data * (path[i][j] - newpath[i][j]) \
                + weight_smooth * (newpath[(i+1) % rowmod][j] - (2 * newpath[i][j]) + newpath[(i-1) % rowmod][j])
...
def printcompare(test, answer):
  ...
  plt.figure()
  plt.scatter(orig_x, orig_y, color='blue')
  plt.scatter(new_x, new_y, color='red')
  plt.draw()


printcompare(testpath1, answer1)
printcompare(testpath2, answer2)
